					...
					<div class="box_descuento2 right col l6 m6 s12">
						...
						<div class="col l12 m12 s12 center" style="margin-top: 3%;">
							...
							<div class="diferencia_box2">
								@if($diferencia!=null)
								Sumando {!! $diferencia !!} productos accedés al descuento de {!! $proximo->porcentaje !!}% en el total
								de tu compra
								@else
				...
				<div class="col s12">
					...
					<table class="highlight bordered">
						...
						<tbody>
							...
							<tr class="celda-contenido">
								<td></td>
								<td class="timagen " style="width: 95px; height: 85px;"><img class="responsive-img"
										src="{{ asset($row->options->imagen) }}" /></td>
							...
							<tr style="border-bottom: none;">
								...
								@php
								$total_iva = $dato_iva*$total_con_descuento;
								@endphp
								<td>{{ '$'.number_format($total_iva, 2, ',','.') }}</td>
								{{ Form::hidden('total_iva', $total_iva) }}
							</tr>
							@php
							$totales = $total_con_descuento+$total_iva;
							@endphp
							<tr style="border-bottom: none;">
								<td colspan="6"></td>
								<td colspan="2" style="font-weight: bold;font-size: 20px;border-bottom: 1px solid;"
									class="total fs24 azul bold">Total (IVA incluido)</td>
								<td style="font-weight: bold;font-size: 20px;border-bottom: 1px solid;">
									{{ '$'.number_format($totales, 2, ',','.') }}</td>
					...
					<div class="row" style="">
						...
						<div class="col s5 right mt30">
							...
							<div class="col s6">
								<button class="enviar" class="bg-azul" href="#modal1" style="color:white; padding: 20px; background-color: #8D302F; border: none; width: 181px;border-radius: 6px; height: 42px!important;"><span style="font-family: 'Montserrat';font-size: 13px;position: relative;bottom: 8px;font-weight: 400;">REALIZAR PEDIDO</span></button></a>
							</div>
							<!-- Modal Structure -->
						...
						<a href="{{ url('/zonaprivada/productos') }}" style="position: relative;right: -20%;cursor: pointer;"
							class="right"><button class="boton seguircomprando"
								style="height: 42px;border: 1px solid #8D302F; color:#8D302F; background-color: white; padding: 20px; width: 181px;position: relative;border-radius: 6px;"><span
									style="font-family: 'Montserrat';font-size: 12px;position: relative;bottom: 8px;font-weight: bold;">CONTINUAR PEDIDO</span></button></a>
					</div>
					@endif
				</div>
			</div>
